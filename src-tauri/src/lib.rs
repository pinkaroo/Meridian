...
fn luau_send_command(text: String, provider: Option<String>) -> Result<(), String> {
    ...
    *LUAU_COMMAND
        ...
        .lock()
        .map_err(|_| "LuaU bridge unavailable.")? =
        Some(serde_json::json!({"text": text, "provider": provider}));
    Ok(())
}

fn start_luau_bridge(app: AppHandle) {
    std::thread::spawn(move || {
        let listener = match TcpListener::bind(("127.0.0.1", 38541)) {
            Ok(value) => value,
            Err(_) => return,
        };
        for stream in listener.incoming() {
            ...
            let payload = if request.starts_with("GET /health")
                || request.starts_with("GET /v1/status")
            {
                r#"{"ok":true,"service":"meridian-luau-bridge","version":1}"#.to_string()
            } else if request.starts_with("GET /v1/command") {
                serde_json::json!({"ok":true,"command":LUAU_COMMAND.get_or_init(|| Mutex::new(None)).lock().unwrap().take()}).to_string()
            } else {
                r#"{"ok":true}"#.to_string()
            };
...
fn save_provider_key(provider: String, api_key: String) -> Result<(), String> {
    if api_key.trim().len() < 8 {
        return Err("API key appears incomplete.".to_string());
    }
...
fn deepseek_api_key() -> Result<String, String> {
    ...
    provider_key("deepseek").map_err(|_| {
        "DEEPSEEK_API_KEY is not configured. Add an official DeepSeek API key in Settings."
            .to_string()
    })
}
...
fn catalog_model(provider: &str, id: String, name: String, access: &str) -> ProviderModel {
    ...
    ProviderModel {
        ...
        tag,
        context_window: None,
        input_cost_usd_per_million: None,
        output_cost_usd_per_million: None,
    }
}
...
async fn fetch_openai_compatible_models(
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|_| format!("{} model catalog could not be reached", provider))?;
    if !response.status().is_success() {
        return Err(format!("{} model catalog request was rejected", provider));
    }
    let value: serde_json::Value = response
        .json()
        .await
        .map_err(|_| format!("{} returned an invalid model catalog", provider))?;
    Ok(value
        .get("data")
        ...
        .filter_map(|item| {
            ...
            model.input_cost_usd_per_million = item
                .get("pricing")
                .and_then(|v| v.get("prompt"))
                .and_then(|v| v.as_str())
                .and_then(|v| v.parse::<f64>().ok())
                .map(|v| v * 1_000_000.0);
            model.output_cost_usd_per_million = item
                .get("pricing")
...
async fn provider_models() -> Result<Vec<ProviderModel>, String> {
    let client = reqwest::Client::builder()
        ...
        .build()
        .map_err(|_| "Could not initialize the provider catalog client".to_string())?;
    let mut models = Vec::new();
    if let Ok(key) = provider_key("openai") {
        if let Ok(mut items) = fetch_openai_compatible_models(
            &client,
            "openai",
                            ...
                            items
                                ...
                                .filter_map(|item| {
                                    ...
                                    model.context_window =
                                        item.get("inputTokenLimit").and_then(|v| v.as_u64());
                                    Some(model)
                                }),
                        );
                    }
...
async fn chat_stream(
    ...
    let (provider, model_type) = requested_model.split_once(':').unwrap_or_else(|| {
        let inferred =
            if requested_model.starts_with("gemini") || requested_model.starts_with("gemma") {
                "google"
    ...
    if provider.is_empty() {
        return Err(format!(
            "The selected model '{}' has no provider. Select a model again in the picker.",
            requested_model
        ));
    }
...
async fn chat_stream_vision(
    ...
    for image in images {
        let encoded = image
            ...
            .unwrap_or(image.content);
        parts.push(
            serde_json::json!({"inline_data": {"mime_type": image.mime_type, "data": encoded}}),
        );
    }
...
async fn merge_single_call(
    ...
    let text = res
        .text()
        .await
        .map_err(|e| format!("{}: read failed: {}", model, e))?;
    ...
    let val: serde_json::Value =
        serde_json::from_str(&text).map_err(|e| format!("{}: parse failed: {}", model, e))?;
...
async fn chat_merge(app: AppHandle, message: String) -> Result<String, String> {
    ...
    let _ = app.emit(
        "merge://done",
        serde_json::json!({ "ok": true, "winner": winner_model.clone() }),
    );
    Ok(winner_text)
}
...
async fn surf_grounded_call(
    ...
    for line in raw.lines() {
        ...
        if let Ok(val) = serde_json::from_str::<serde_json::Value>(json_str) {
            ...
            let results_key = if val.get("results").is_some() {
                "results"
            } else {
                "sources"
            };
...
fn validate_skill_dir(dir: &Path) -> Result<String, String> {
    ...
    let content =
        fs::read_to_string(&skill_md).map_err(|e| format!("Could not read SKILL.md: {}", e))?;
...
async fn ensure_default_skills(
    ...
    tokio::task::spawn_blocking(move || -> Result<Vec<String>, String> {
        ...
        for entry in
            fs::read_dir(&bundled_skills).map_err(|e| format!("read bundled skills: {}", e))?
        {
            let entry = entry.map_err(|e| e.to_string())?;
...
async fn import_skill(source_dir: String, target_root: String) -> Result<String, String> {
    tokio::task::spawn_blocking(move || -> Result<String, String> {
        ...
        if dest.exists() {
            return Err(format!(
                "Skill '{}' already exists. Delete it first to re-import.",
                name
            ));
        }
